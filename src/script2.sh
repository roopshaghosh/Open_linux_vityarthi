...
# Author: Roopsha | Reg No: 24BCE10292
...
if command -v rpm &>/dev/null && rpm -q "$PACKAGE" &>/dev/null; then
    ...
    echo "STATUS: Package '$PACKAGE' is installed on this system (Debian-based)"
    echo ""
...
case $PACKAGE in
    git)
        echo "Git: Developed by Linus Torvalds, Git revolutionized version control"
    ...
    libreoffice)
        ...
        ;;
    *)
        echo "Open-source software like '$PACKAGE' promotes collaboration,"
echo "transparency, and innovation across the global developer community."
